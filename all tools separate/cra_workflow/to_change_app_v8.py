...
def render_pillar1_deep_dive_dashboard(df: pd.DataFrame, max_items: int) -> None:
    ...
    e1.warning(f"🦋 **{low_util}** contracts below 70% utilization — optimization review task created")
...
def render_actions_dashboard(df: pd.DataFrame, view_level: str, max_items: int) -> None:
    ...
    c3.metric("Average Portfolio Utilization", f"{df['Avg_Utilization_Pct'].mean():.1f}%")
    c4.metric("Budget Overusage", int((df["Budget_Status"] == "Exceeded").sum()))
    ...
    usage["color"] = np.where(
        (usage["Avg_Utilization_Pct"] < 70) | (usage["Avg_Utilization_Pct"] > 90),
        COLOR_RED,
        COLOR_GREEN,
    )
    fig1 = go.Figure(
    ...
    st.markdown(
        ...
        f"<span><span class='legend-dot' style='background:{COLOR_RED}'></span>Under-utilized (&lt;70%) or over-utilized (&gt;90%)</span>"
        f"<span><span class='legend-dot' style='background:{COLOR_GREEN}'></span>Balanced (70%-90%)</span>"
        "</div>",
        unsafe_allow_html=True,
    )
    ...
    a1.warning(f"🦋 **{n_low}** items below 70% utilization — optimization review task created")
    ...
    import json
    import os
    override_file = "agent_state.json"
